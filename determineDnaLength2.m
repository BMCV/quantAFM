...
%     if( ~(size(currPxlList,1) < 3)  && ~(size(currPxlList,1) > 210)) 
	...
	% if config param is set
        if (RECOVERBACKBONE)
            ...
            % need this for the potential "mediate length" parameter
            if (AVERAGELENGTH)
                singlePath_short = currPxlList(singlePath);
            end
        ...
        if (AVERAGELENGTH)
            bwImgThinnedRemoved(singlePath_short) = 1;
            dnaObj.bwImageThinnedRemovedShort = bwImgThinnedRemoved;
        end
        % we can reuse the same image since the "longer one" has to cover
        ...
        % If desired, calculate the average length estimate between the
        % elongated and "regular" thinned object.
        if (AVERAGELENGTH)
            short_length = calcKulpaLength(singlePath_short, bwImgThinnedRemoved);
            fragmentLen{1} = (fragmentLen{1}+short_length)/2;
        end
        ...
        if(dnaHasNucleos && numel(dnaObj.attachedNukleo) == 1)
            arms = getArmsNucleoIntersection(dnaObj, 0); %%% check whether this code works with > 1 nucleosome
            ...
            if(arms.NumObjects == 0 || arms.NumObjects ~= arms_short.NumObjects)
                ...
                for armIdx = 1:arms.NumObjects % for each arm, ...
                    ...
                    if (AVERAGELENGTH) % ... and average, if wanted
                        currArm_short = arms_short.PixelIdxList{armIdx};
                        fragmentLen{armIdx+1} = (fragmentLen{armIdx+1}+...
                            calcKulpaLength(currArm_short, bwImgThinnedRemoved))/2;
                    end
                end
            end

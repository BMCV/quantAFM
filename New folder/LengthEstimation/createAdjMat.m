...
for i = 1:length(pixels)
    ...
    if pixels(pixels == currentPixel - height - 1)
        adjMat(find(pixels == currentPixel - height - 1), i) = 1;
    end
    % connected to the left pixel?
    if pixels(pixels == currentPixel - height)
        adjMat(find(pixels == currentPixel - height), i) = 2;
    end
    % connected to the lower left pixel?
    if pixels(pixels == currentPixel - height + 1)
        adjMat(find(pixels == currentPixel - height + 1), i) = 3;
    end
    % connected to the upper pixel?
    if pixels(pixels == currentPixel - 1)
        adjMat(find(pixels == currentPixel - 1), i) = 8;
    end
    % connected to the lower pixel?
    if pixels(pixels == currentPixel + 1)
        adjMat(find(pixels == currentPixel + 1), i) = 4;
    end
    % connected to the upper right pixel?
    if pixels(pixels == currentPixel + height - 1)
        adjMat(find(pixels == currentPixel + height - 1), i) = 7;
    end
    % connected to the right pixel?
    if pixels(pixels == currentPixel + height)
        adjMat(find(pixels == currentPixel + height), i) = 6;
    end
    % connected to the lower right pixel?
    if pixels(pixels == currentPixel + height + 1)
        adjMat(find(pixels == currentPixel + height + 1), i) = 5;
    end
end
